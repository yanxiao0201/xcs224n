...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for s in sents:
            ...
            for w in s:
                w_ids = []
                w_ids.append(self.start_of_word)
                for c in w:
                    w_ids.append(self.char2id[c])
                w_ids.append(self.end_of_word)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_indices = pad_sents_char(indices, self.char2id['<pad>'])  #(batch_size, max_sentence_length, max_word_length)

        new_tensor = torch.tensor(padded_indices, device=device)
