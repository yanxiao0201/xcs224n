...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        packed_x = nn.utils.rnn.pack_padded_sequence(x, source_lengths)
        enc_hiddens_packed, (last_hidden, last_cell) = self.encoder(packed_x)
        enc_hiddens = nn.utils.rnn.pad_packed_sequence(enc_hiddens_packed, batch_first=True)[0]
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        tmp_e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, 2))
        e_t = torch.squeeze(tmp_e_t, 2)
